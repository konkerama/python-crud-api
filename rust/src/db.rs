...
impl DB {
    pub async fn init() -> Result<Self> { 
        let mongodb_username: String = std::env::var("ME_CONFIG_MONGODB_ADMINUSERNAME").expect("DATABASE_URL must be set.");
        let mongodb_passwd: String = std::env::var("ME_CONFIG_MONGODB_ADMINPASSWORD").expect("DATABASE_URL must be set.");
        let mongodb_server: String = std::env::var("ME_CONFIG_MONGODB_SERVER").expect("DATABASE_URL must be set.");

        let mongodb_uri = format!("mongodb://{}:{}@{}/", mongodb_username, mongodb_passwd,mongodb_server);								
